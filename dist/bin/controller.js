...
export function printUsage(binName) {
    console.log(`
💡 Usage Guide:
...
export async function listTasks(args, binName) {
...
export async function restoreTask(idArg) {
    const id = Number(idArg);
    if (isNaN(id)) {
        console.error("❌ Error: Valid ID required.");
        process.exitCode = 1;
...
export async function toggleTask(idArg) {
    const id = Number(idArg);
    if (isNaN(id)) {
        console.error("❌ Error: Valid ID required.");
        process.exitCode = 1;
